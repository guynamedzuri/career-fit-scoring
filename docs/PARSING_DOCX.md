...
**DOCX 파싱에서는 정규표현식을 사용하지 않습니다.**  
테이블 인덱스 + 행/열 위치(`tableIndex`, `rowIndex`, `cellIndex`)로 셀 텍스트만 읽고, `resumeMapping.ts`의 매핑 설정에 따라 필드에 넣습니다.  
다른 구현에서 “문자열 패턴”이 필요하면 PDF 파싱 문서(`PARSING_PDF.md`)의 정규식만 참고하면 됩니다.
