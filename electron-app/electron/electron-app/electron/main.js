        ...
        if (process.resourcesPath) {
            ...
            if (fs.existsSync(unpackedPath)) {
                ...
                try {
                    ...
                    // 절대 경로로도 시도
                    try {
                        const asarUpdaterPath = path.join(electron_1.app.getAppPath(), 'node_modules', 'electron-updater');
...
function setupAutoUpdater() {
    ...
    if (!autoUpdater) {
        ...
        if (updaterModule && updaterModule.autoUpdater) {
            ...
            if (electron_1.app.isReady()) {
                writeLog(`[AutoUpdater] app.getAppPath(): ${electron_1.app.getAppPath()}`, 'error');
                writeLog(`[AutoUpdater] process.resourcesPath: ${process.resourcesPath || 'undefined'}`, 'error');
            }
            if (mainWindow) {
                electron_1.dialog.showErrorBox('자동 업데이트 오류', 'electron-updater를 로드할 수 없습니다.\n자동 업데이트가 비활성화됩니다.');
            }
            return;
        }
    }
    ...
    try {
        ...
        writeLog(errorMsg, 'error');
        if (mainWindow) {
            electron_1.dialog.showErrorBox('업데이트 설정 오류', `업데이트 서버를 설정하는 중 오류가 발생했습니다:\n\n${error.message || error}`);
        }
    }
    ...
    autoUpdater.checkForUpdatesAndNotify().catch((error) => {
        ...
        writeLog(errorMsg, 'error');
        if (mainWindow) {
            electron_1.dialog.showErrorBox('업데이트 확인 실패', `업데이트를 확인하는 중 오류가 발생했습니다:\n\n${error.message || error}\n\n로그 파일: ${path.join(electron_1.app.getPath('userData'), 'logs', 'app.log')}`);
        }
    });
    // 업데이트 체크 주기 설정 (5분마다)
    setInterval(() => {
    ...
    autoUpdater.on('error', (err) => {
        ...
        setTimeout(() => {
            const logPath = path.join(electron_1.app.getPath('userData'), 'logs', 'app.log');
            electron_1.dialog.showErrorBox('업데이트 확인 오류', `업데이트를 확인하는 중 오류가 발생했습니다:\n\n${errorMessage}\n\n자세한 내용은 로그 파일을 확인하세요:\n${logPath}`);
...
function createWindow() {
    ...
    console.log('isDev:', isDev, 'isPackaged:', electron_1.app.isPackaged);
...
// 캐시 파일 저장
function saveCache(folderPath, cacheData) {
    try {
        const cachePath = getCacheFilePath(folderPath);
...
electron_1.ipcMain.handle('get-docx-files', async (event, folderPath) => {
    try {
...
// 캐시 데이터 로드 IPC 핸들러
electron_1.ipcMain.handle('load-cache', async (event, folderPath, filePaths) => {
    try {
        if (!folderPath || !fs.existsSync(folderPath)) {
...
// 캐시 데이터 저장 IPC 핸들러
electron_1.ipcMain.handle('save-cache', async (event, folderPath, results) => {
    try {
        if (!folderPath || !fs.existsSync(folderPath)) {
            return;
        }
...
function getCareerNetBackupPath() {
    const projectRoot = findProjectRoot();
    if (projectRoot) {
...
// Q-Net 비상용 로컬 데이터 저장
function saveQNetBackup(certifications) {
    try {
...
function saveCareerNetBackup(jobs) {
    try {
...
electron_1.ipcMain.handle('qnet-search-certifications', async () => {
...
electron_1.ipcMain.handle('read-official-certificates', async () => {
    try {
        ...
        console.log('[Official Certs IPC] app.getAppPath():', electron_1.app.getAppPath());
        // 프로젝트 루트 찾기
        const projectRoot = findProjectRoot();
        if (projectRoot) {
            ...
            if (fs.existsSync(filePath)) {
                ...
                const encodings = ['utf-8', 'utf8', 'euc-kr', 'cp949'];
...
// 이력서 처리 IPC 핸들러
electron_1.ipcMain.handle('process-resume', async (event, filePath) => {
    try {
        // DOCX 파일에서 테이블 추출
        const tables = await (0, index_1.extractTablesFromDocx)(filePath);
        // 매핑 설정으로 applicationData 변환
        const applicationData = (0, index_1.mapResumeDataToApplicationData)(tables);
...
electron_1.ipcMain.handle('ai-check-resume', async (event, data) => {
    try {
...
function formatResumeDataForAI(applicationData) {
    if (!applicationData)
        return '이력서 데이터가 없습니다.';
    let text = '';
    // 기본 정보
    if (applicationData.name)
        text += `이름: ${applicationData.name}\n`;
    if (applicationData.birthDate)
        text += `생년월일: ${applicationData.birthDate}\n`;
    if (applicationData.email)
        text += `이메일: ${applicationData.email}\n`;
    if (applicationData.phone)
        text += `전화번호: ${applicationData.phone}\n`;
    text += '\n';
    // 자격증
    const certificates = [];
    for (let i = 1; i <= 10; i++) {
...
electron_1.ipcMain.handle('careernet-search-jobs', async () => {
    return new Promise((resolve, reject) => {
        // .env 파일 로드
        loadEnvFile();
...
electron_1.ipcMain.handle('careernet-get-job-detail', async (event, jobdicSeq) => {
    return new Promise((resolve, reject) => {
        // .env 파일 로드
        loadEnvFile();
        const apiKey = process.env.CAREERNET_API_KEY || '83ae558eb34c7d75e2bde972db504fd5';
