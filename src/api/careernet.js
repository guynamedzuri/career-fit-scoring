...
exports.searchMajors = searchMajors;
/**
 ...
 * @returns 직종 목록
 */
async function searchJobs(apiKey) {
 ...
 * @returns 직종 상세 정보
 */
async function getJobDetail(jobdicSeq, apiKey) {
    const key = apiKey || CAREERNET_API_KEY;
 ...
 * @returns 학과 목록
 */
async function searchMajors(gubun = 'univ_list', apiKey) {
    const key = apiKey || CAREERNET_API_KEY;
    ...
    try {
        ...
        contentList.forEach((item) => {
            if (item.facilName) {
                ...
                const facilNames = item.facilName.split(',').map((name) => name.trim()).filter((name) => name);
                facilNames.forEach((name) => {
                    majors.push({
                        facilName: name,
