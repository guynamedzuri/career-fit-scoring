 ...
 */
export function parseOfficialCertificates(fileContent: string): string[] {
 ...
 * @returns 자격증 이름 배열
 */
export function parseAdditionalNationalCertificates(certList: string): string[] {
  return certList
    .split('\n')
    .map(line => line.trim())
 ...
 */
export const ADDITIONAL_NATIONAL_CERTIFICATES = `
전파통신기사
전파통신산업기사
전파통신기능사
전파전자통신기사
