...
exports.DEFAULT_RESUME_MAPPING = void 0;
exports.classifyResidence = classifyResidence;
exports.mapResumeDataToApplicationData = mapResumeDataToApplicationData;
const docxParser_1 = require("./docxParser");
/**
 * 기본 매핑 설정
 * resume_form.docx 양식에 맞게 설정됨
 */
exports.DEFAULT_RESUME_MAPPING = {
    basicInfo: {
 ...
 */
function resolveCellPosition(tables, tableIndex, position, defaultRowIndex) {
    if (!position)
        return null;
    ...
    if ('searchRow' in position && 'searchCol' in position) {
        // 검색 텍스트로 찾기
        const rowIndex = (0, docxParser_1.findRowByText)(tables, tableIndex, position.searchRow);
        if (rowIndex === -1)
            return null;
        const cellIndex = (0, docxParser_1.findColumnByText)(tables, tableIndex, position.searchCol, rowIndex);
        if (cellIndex === -1)
            return null;
 ...
 */
function resolveColumnIndex(tables, tableIndex, position, headerRowIndex = 0) {
    ...
    if ('searchText' in position) {
        return (0, docxParser_1.findColumnByText)(tables, tableIndex, position.searchText, headerRowIndex);
    }
    return -1;
}
/**
 ...
 */
function safeGetCellValue(tables, tableIndex, rowIndex, cellIndex) {
    try {
        const value = (0, docxParser_1.getCellValue)(tables, tableIndex, rowIndex, cellIndex);
 ...
 */
function classifyResidence(address) {
    if (!address)
        return undefined;
 ...
 */
function mapResumeDataToApplicationData(tables, mappingConfig = exports.DEFAULT_RESUME_MAPPING) {
    ...
    if (education.tableIndex >= 0 && education.tableIndex < tables.length) {
        ...
        for (let rowIdx = dataStartRowIndex; rowIdx < endRow && eduIndex <= maxCount; rowIdx++) {
            ...
            if (startDate)
                applicationData[`educationStartDate${eduIndex}`] = startDate;
            // 졸업년월 (cell 1)
            const endDate = safeGetCellValue(tables, education.tableIndex, rowIdx, 1);
            if (endDate)
                applicationData[`educationEndDate${eduIndex}`] = endDate;
            // 학교명 (cell 2)
            if (schoolNameCol >= 0 && schoolNameCol < row.cells.length) {
                ...
                if (schoolName) {
                    ...
                    if (location)
                        applicationData[`universityLocation${eduIndex}`] = location;
                    // 졸업구분 (cell 6)
        ...
        for (let rowIdx = dataStartRowIndex; rowIdx < table.rows.length && careerIndex <= maxCount; rowIdx++) {
            ...
            if (companyCol >= 0 && companyCol < row.cells.length) {
                ...
                if (companyName) {
                    ...
                    if (department)
                        applicationData[`careerDepartment${careerIndex}`] = department;
                    ...
                    if (salary)
                        applicationData[`careerSalary${careerIndex}`] = salary;
                ...
                if (countryCol >= 0 && countryCol < row.cells.length) {
                    ...
                    if (country) {
                        ...
                        if (durationCol >= 0 && durationCol < row.cells.length) {
                            ...
                            if (duration)
                                applicationData[`overseasTrainingDuration${trainingIndex}`] = duration;
                        }
                        if (purposeCol >= 0 && purposeCol < row.cells.length) {
                            const purpose = safeGetCellValue(tables, training.tableIndex, rowIdx, purposeCol);
                            if (purpose)
                                applicationData[`overseasTrainingPurpose${trainingIndex}`] = purpose;
                        }
                        trainingIndex++;
                    }
                }
        ...
        if (careerDetails.tableIndex >= 0 && careerDetails.tableIndex < tables.length) {
            ...
            careerDetails.blocks.slice(0, maxCount).forEach((block, index) => {
                ...
                if (block.detailRowIndex < table.rows.length) {
                    ...
                    if (detail)
                        applicationData[`careerDetailDescription${detailIndex}`] = detail;
                }
            });
        }
    }
