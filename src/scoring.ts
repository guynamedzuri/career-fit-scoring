...
export interface CertificationScoreParams {
  applicantCertifications: string[]; // 지원자가 가진 자격증 목록
  requiredCertifications: string[]; // 필수 자격증 목록
  relatedCertifications: string[]; // 관련 자격증 목록 (커리어넷 API에서 가져온 것)
}
 ...
 */
function calculateMatchScore(cert: string, target: string): number {
 ...
 * - 최대 10점
 */
export function calculateCertificationScore(params: CertificationScoreParams): number {
  ...
  if (requiredCertifications.length === 0) {
    ...
    const matchedRequired = applicantCertifications.filter(cert => 
      requiredCertifications.some(req => {
...
export interface CareerInfo {
  jobTypeName?: string; // 직종 이름
  jobTypeAptdCode?: string; // 직종 aptd_type_code
  jobTypeCode?: string; // 직종 코드 (jobdicSeq)
  startDate: Date | null; // 근무 시작일
  endDate: Date | null; // 근무 종료일
  employmentStatus?: string; // 재직중/퇴사
...
export function calculateCareerScore(
  applicantCareers: CareerInfo[],
  jobAptdCode: string | undefined,
  jobdicSeq: string | undefined,
  applicantAge: number
): number {
  ...
  const calculateEmploymentPeriod = (startDate: Date, endDate: Date): { totalMonths: number } => {
    // 년월 차이 계산
...
export interface EducationInfo {
  degreeType: '고졸' | '전문학사' | '학사' | '석사' | '박사';
  schoolName?: string; // 학교명
  graduationType?: string; // 졸업 여부 (졸업, 졸업예정, 재학중, 휴학, 중퇴 등)
  majorSeq?: string; // 학과 코드 (MAJOR_SEQ)
  majorNm?: string; // 학과명 (MAJOR_NM)
  gpa?: number; // 학점 (GPA)
  maxGpa?: number; // 만점 (최대 학점)
}
...
export function calculateEducationScore(
  applicantEducations: EducationInfo[],
  jobMajorNm?: string,
  jobMajorSeq?: string,
  jobRelatedMajors?: Array<{ majorNm: string; majorSeq: string }>
): number {
  ...
  // 2. 관련 학과 점수 계산
  const relatedMajorScore = calculateRelatedMajorScore(
    applicantEducations, 
    jobMajorNm, 
    jobMajorSeq,
    jobRelatedMajors
 ...
 */
export function calculateRelatedMajorScore(
  applicantEducations: EducationInfo[],
  jobMajorNm: string | undefined,
  jobMajorSeq: string | undefined,
  jobRelatedMajors?: Array<{ majorNm: string; majorSeq: string }>
): number {
  if (!applicantEducations || applicantEducations.length === 0) {
    return 0;
  ...
  const relatedMajors: Array<{ majorNm: string; majorSeq: string }> = [];
 ...
 */
export function calculateGpaScore(applicantEducations: EducationInfo[]): number {
  if (!applicantEducations || applicantEducations.length === 0) {
    return 0;
  }
 ...
 * @returns 자격증 이름 배열
 */
export function extractCertifications(applicationData: any): string[] {
 ...
 * @returns 경력 정보 배열
 */
export function extractCareers(applicationData: any): CareerInfo[] {
 ...
 * @returns 학력 정보 배열
 */
export function extractEducations(applicationData: any): EducationInfo[] {
